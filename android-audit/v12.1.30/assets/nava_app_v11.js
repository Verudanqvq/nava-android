...
/* Nava v12.1.30 — Firestore-safe Android push token registration. */
;(function(d,w){
  'use strict';
  if(w.__navaAndroidPushV12130)return;
  w.__navaAndroidPushV12130=true;
  ...
  function flagKey(uid){return'nava_push_registered_v12130:'+String(uid||'')}
  function writeCreate(ref,uid,token){var stamp=firebase.firestore.FieldValue.serverTimestamp();return ref.set({uid:uid,token:token,platform:'android',appVersion:'12.1.30',deviceKey:deviceKey,createdAt:stamp,updatedAt:stamp},{merge:false})}
  function writeUpdate(ref,uid,token){var stamp=firebase.firestore.FieldValue.serverTimestamp();return ref.set({uid:uid,token:token,platform:'android',appVersion:'12.1.30',deviceKey:deviceKey,updatedAt:stamp},{merge:true})}
  function registerNow(){
    if(!currentUser||!currentToken)return;if(!readyForFirestore()){scheduleRegister(350);return}
    var uid=currentUser.uid,token=currentToken,ref=w.db.collection('devicePushTokens').doc(accountInstallId(uid)),preferUpdate=false;
