...
NEW_VERSION='12.1.29'
NEW_CODE=45
OLD_PREF=b'nava_notification_permission'
NEW_PREF=b'nava_notif_permission_v12129'
...
def patch_js(original,replacement):
    marker='/* Nava v12.1.28 — Firestore-safe Android push token registration.'
    start=original.find(marker)
    if start<0: raise ValueError('12.1.28 push marker missing')
    end_marker='})(document,window);'; end=original.find(end_marker,start)
    if end<0: raise ValueError('push block end missing')
    end+=len(end_marker)
    out=original[:start]+replacement.rstrip()+original[end:]
    if "appVersion:'12.1.29'" not in out: raise ValueError('12.1.29 appVersion missing')
    if '.get().then(function(snapshot)' in replacement or 'ref.get()' in replacement: raise ValueError('forbidden token pre-read')
    return out

def patch_dex(raw):
    if len(OLD_PREF)!=len(NEW_PREF): raise ValueError('DEX replacement length mismatch')
    if raw.count(OLD_PREF)!=1: raise ValueError(f'notification preference marker count={raw.count(OLD_PREF)}')
    d=bytearray(raw.replace(OLD_PREF,NEW_PREF,1))
    # DEX signature covers bytes 32..end; checksum covers bytes 12..end including the new signature.
    d[12:32]=hashlib.sha1(d[32:]).digest()
    struct.pack_into('<I',d,8,zlib.adler32(d[12:])&0xffffffff)
    if bytes(d).count(OLD_PREF)!=0 or bytes(d).count(NEW_PREF)!=1: raise ValueError('DEX permission-key patch verify failed')
    return bytes(d)
...
def main():
    if len(sys.argv)!=5: raise SystemExit('usage: patch INPUT.apk PUSH.js OUTPUT.apk CODE')
    source=Path(sys.argv[1]); replacement=Path(sys.argv[2]).read_text(); target=Path(sys.argv[3]); code=int(sys.argv[4])
    ...
    with zipfile.ZipFile(source,'r') as zin:
        ...
        js=patch_js(zin.read('assets/nava_app_v11.js').decode(),replacement).encode()
    ...
    with zipfile.ZipFile(source,'r') as a, zipfile.ZipFile(target,'r') as b:
        ...
        final_dex=b.read('classes.dex'); final_js=b.read('assets/nava_app_v11.js').decode()
        if final_dex.count(NEW_PREF)!=1 or final_dex.count(OLD_PREF): raise ValueError('final dex failed')
        if "appVersion:'12.1.29'" not in final_js: raise ValueError('final JS failed')
    ...
    print('PATCH_OK versionCode=45 js=12.1.29 dex=permission-reprompt unchanged_entries=ok')
