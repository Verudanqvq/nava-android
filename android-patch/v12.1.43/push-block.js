/* Nava v12.1.43 — direct native FCM registration. */
;(function(d,w){
  'use strict';
  if(w.__navaAndroidPushV12143)return;
  w.__navaAndroidPushV12143=true;
  ...
  function registerNow(){
    ...
    ref.set({uid:uid,token:token,platform:'android',appVersion:'12.1.43',deviceKey:deviceKey,notificationProtocol:'direct-native-v5',notificationChannel:'nava_follower_releases_v4',updatedAt:stamp},{merge:true}).then(function(){
      try{localStorage.setItem('nava_push_registered_v12143:'+uid,'1')}catch(_){}
      if(verifyTimer)clearTimeout(verifyTimer);verifyTimer=setTimeout(function(){if(currentUser)requestNativeToken()},60000);
    }).catch(function(error){try{console.error('Nava push token kaydı başarısız:',error)}catch(_){}schedule(3500)})
  }
  w.navaAndroidPushToken=function(token){currentToken=String(token||'').trim();if(currentToken)registerNow()};
  w.navaAndroidPushTokenError=function(){schedule(3500)};
  w.navaAndroidEnsurePushToken=function(){if(currentUser)requestNativeToken();else bindAuth();return true};
  function burst(){requestNativeToken();setTimeout(requestNativeToken,700);setTimeout(requestNativeToken,2200)}
  ...
  w.addEventListener('pageshow',function(){setTimeout(function(){if(currentUser)requestNativeToken()},300)});
  d.addEventListener('visibilitychange',function(){if(!d.hidden&&currentUser)setTimeout(requestNativeToken,220)});
  w.addEventListener('online',function(){if(currentUser)setTimeout(requestNativeToken,300)});
  bindAuth();
})(document,window);
