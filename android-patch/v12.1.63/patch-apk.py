...
OLD_VERSION='12.1.62';NEW_VERSION='12.1.63';NEW_CODE=79
...
def main():
 if len(sys.argv)!=6:raise SystemExit('usage: patch-apk.py SRC CLASSES2 LIBJS OFFLINE OUT')
 src,c2f,jsf,offlinef,out=sys.argv[1:];srcp=Path(src);got=hashlib.sha256(srcp.read_bytes()).hexdigest()
 if got.lower()!=SOURCE_SHA:raise ValueError('source must be signed Nava 12.1.62; sha256='+got)
 newc2=Path(c2f).read_bytes();addjs=Path(jsf).read_text(encoding='utf-8').strip();offline=Path(offlinef).read_bytes()
 ...
 for t in (b'submitBatch63',b'seriesRelations63',b'Nava:OfflineDownload63'):
  if t not in newc2:raise ValueError('classes2 token missing '+t.decode('utf-8','ignore'))
 with zipfile.ZipFile(srcp) as zin:
  ...
  manifest=patch_manifest(zin.read('AndroidManifest.xml'));c1=zin.read('classes.dex');oldc2=zin.read('classes2.dex');c3=zin.read('classes3.dex') if 'classes3.dex' in names else None;res=zin.read('resources.arsc')
 ...
 with zipfile.ZipFile(out) as z:
  ...
  for t in (JS_MARK,'__navaDownloadedLibraryV12163','getSeriesRelations','setSeriesRelations','looksVolume','refreshRelations'):
 ...
 print('PATCH_OK versionName=12.1.63 versionCode=79 base=12.1.62 library=relation-aware background=wakelock native=classes2 webview=preserved')
