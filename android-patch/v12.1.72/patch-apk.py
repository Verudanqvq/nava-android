...
def main():
    ...
    with zipfile.ZipFile(src) as zin:
        ...
        js=once(js,oldmeta,newmeta,'downloaded metadata fields')
        js=once(js,"if(!junk(st))series=strip(st);","if(!junk(sv))series=sv;else if(!junk(st))series=strip(st);",'downloaded explicit series')
        js=once(js,"open=q?true:!!state.seriesOpen[sk]","open=!!state.seriesOpen[sk]",'search series compact')
        js=once(js,"openV=q?true:!!state.volumeOpen[vk]","openV=!!state.volumeOpen[vk]",'search volume compact')
        ...
        js=once(js,oldsort,newsort,'downloaded volume zero sort')
        js=js.rstrip()+'\n'+overlay.rstrip()+'\n'
    ...
    with zipfile.ZipFile(out) as z:
        ...
        for token in ('__navaDownloadStateV12172','getDownloadQueue72','seriesName','volumeNo','chapterNo','groupKey','open=!!state.seriesOpen[sk]','openV=!!state.volumeOpen[vk]'):
