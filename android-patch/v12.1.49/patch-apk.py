...
OLD_VERSION='12.1.47';NEW_VERSION='12.1.49';NEW_CODE=65
...
def patch_manifest(raw):
 ...
 return bytes(d)
def patch_dex(raw):
 d=bytearray(raw);old=b'NavaAndroidApp/12.1.47';new=b'NavaAndroidApp/12.1.49'
 if d.count(old)<1:raise ValueError('12.1.47 UA marker missing from classes.dex')
 d[:]=d.replace(old,new);d[12:32]=hashlib.sha1(d[32:]).digest();struct.pack_into('<I',d,8,zlib.adler32(d[12:])&0xffffffff);return bytes(d)
...
def main():
 if len(sys.argv)!=8:raise SystemExit('usage: patch-apk.py SRC_12.1.47 MENU OFFLINE LANG NOTIFY CSS OUT')
 src,menuf,offf,langf,notifyf,cssf,out=sys.argv[1:];srcp=Path(src);got=hashlib.sha256(srcp.read_bytes()).hexdigest()
 if got.lower()!=SOURCE_SHA:raise ValueError('source must be signed Nava 12.1.47; sha256='+got)
 menu=Path(menuf).read_text(encoding='utf-8');offline=Path(offf).read_text(encoding='utf-8');lang=Path(langf).read_text(encoding='utf-8');notify=Path(notifyf).read_text(encoding='utf-8');css=Path(cssf).read_text(encoding='utf-8')
 for text,mark in((menu,NEW_MENU),(offline,NEW_OFFLINE),(lang,NEW_LANG),(notify,NOTIFY),(css,CSS_MARK)):
 ...
 with zipfile.ZipFile(srcp) as zin:
  ...
  manifest=patch_manifest(zin.read('AndroidManifest.xml'));dex=patch_dex(zin.read('classes.dex'));js=zin.read('assets/nava_app_v11.js').decode('utf-8');js=replace_iife(js,OLD_MENU,menu);js=replace_iife(js,OLD_OFFLINE,offline);js=replace_iife(js,OLD_LANG,lang)
  if NOTIFY in js:raise ValueError('notification patch already present')
 ...
 with zipfile.ZipFile(out) as z:
  ...
  for mark in(NEW_MENU,NEW_OFFLINE,NEW_LANG,NOTIFY):
   if mark not in fj:raise ValueError('final js marker missing '+mark)
  for mark in(OLD_MENU,OLD_OFFLINE,OLD_LANG):
   if mark in fj:raise ValueError('old 47 block remains '+mark)
  for token in('nava-series-download-inline-v12149','nava-download-hub-v12149','nava_download_queue_v12149','nava-series-language-v12149','nava-notification-clear-all-v12149','nava-offline-browser-v12149'):
   if token not in fj+fc:raise ValueError('required feature missing '+token)
  if b'NavaAndroidApp/12.1.49' not in fd:raise ValueError('UA patch missing')
  if CSS_MARK not in fc:raise ValueError('css patch missing')
 print('PATCH_OK versionName=12.1.49 versionCode=65 base=12.1.47 download=series-picker hub=queue+library language=per-series notifications=delete offline=robust-delete')
