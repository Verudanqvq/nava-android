...
NEW_VERSION='12.1.41'
NEW_CODE=57
OLD_PUSH='/* Nava v12.1.40 — direct native FCM registration. */'
NEW_PUSH='/* Nava v12.1.41 — direct native FCM registration. */'
OLD_OFFLINE='/* Nava Android v12.1.31 — native offline downloads UI. */'
NEW_OFFLINE='/* Nava Android 12.1.41 — hierarchical offline library. */'
OLD_MENU='/* Nava Android 12.1.38 — clearer download center + all volumes. */'
NEW_MENU='/* Nava Android 12.1.41 — download center metadata + content-type neutral hierarchy. */'
CSS_MARKER='/* Nava Android 12.1.41 — hierarchical offline library styles. */'
...
def main():
    if len(sys.argv)!=10: raise SystemExit('usage: patch-apk.py SRC DEX PUSH OFFLINE_JS MENU_JS OFFLINE_CSS OFFLINE_HTML OUT EXPECTED_SHA')
    ...
    dex=Path(dexf).read_bytes();push=Path(pushf).read_text();offlinejs=Path(offlinejsf).read_text();menujs=Path(menuf).read_text();newcss=Path(cssf).read_text();offline=Path(offlinef).read_bytes()
    if not dex.startswith(b'dex\n') or b'NavaAndroidApp/12.1.41' not in dex: raise ValueError('patched dex invalid')
    if NEW_PUSH not in push or "appVersion:'12.1.41'" not in push: raise ValueError('push source invalid')
    if NEW_OFFLINE not in offlinejs or 'seriesOpen' not in offlinejs or 'volumeOpen' not in offlinejs: raise ValueError('offline JS invalid')
    if NEW_MENU not in menujs or 'data-nava-chapter-meta' not in menujs or 'downloadGroup' not in menujs: raise ValueError('download menu invalid')
    if CSS_MARKER not in newcss: raise ValueError('offline CSS marker missing')
    html=offline.decode('utf-8')
    if '<h1>İndirilenler</h1>' not in html or 'data-series' not in html or 'data-volume' not in html: raise ValueError('offline HTML hierarchy invalid')
    ...
    with zipfile.ZipFile(srcp) as zin:
        ...
        js=replace_iife(js,OLD_MENU,menujs)
    ...
    with zipfile.ZipFile(srcp) as a,zipfile.ZipFile(out) as b:
        ...
        if OLD_OFFLINE in fj or OLD_MENU in fj: raise ValueError('old offline UI blocks remain')
        if CSS_MARKER not in fc: raise ValueError('final offline CSS missing')
        if 'Tekrar bağlan' in fh or 'Yalnız Wi' in fh or 'data-series' not in fh or 'data-volume' not in fh: raise ValueError('final offline HTML invalid')
        for token in ('#6d28d9','#7c3aed','#8b5cf6','#5b20f3','#4c1d95','#c4b5fd','#ddd6fe','#ede9fe','#f5f3ff'):
            if token.lower() in newcss.lower() or token.lower() in fh.lower(): raise ValueError('purple token in new offline UI '+token)
        if b'NavaAndroidApp/12.1.41' not in fd: raise ValueError('UA missing')
    ...
    print('PATCH_OK versionCode=57 versionName=12.1.41 offline=series-volume-chapter-collapsible metadata=content-neutral palette=blue-gray scoped=ok')
