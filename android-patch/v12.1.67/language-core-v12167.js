/* Nava Android 12.1.67 TEST-FIRST — pure current-chapter language core. */
(function(root,factory){
  var api=factory();
  if(typeof module==='object'&&module.exports)module.exports=api;
  if(root)root.NavaLanguageCoreV12167=api;
...
function buildIndex(entries){
  ...
  (entries||[]).forEach(function(e){
    var url=entryUrl(e);if(!url)return;
    var labels=entryLabels(e),n=chapterNumber(e);if(!n)return;
    var vl=volumeLabel(labels),lang=explicitLang(labels)||'TR';
    var rec={url:url,title:entryTitle(e),labels:labels,chapterNo:String(n),volumeLabel:vl,volumeKey:norm(vl),lang:lang};
    byUrl[url]=rec;
    var key=(rec.volumeKey||'novol')+'|'+rec.chapterNo;if(!groups[key])groups[key]=[];groups[key].push(rec);
  });
  return{byUrl:byUrl,groups:groups};
}
function variantsForUrl(index,currentUrl){
  index=index||{byUrl:{},groups:{}};var u=canon(currentUrl),cur=index.byUrl&&index.byUrl[u];if(!cur)return{current:null,languages:[],targets:{},show:false};
  var key=(cur.volumeKey||'novol')+'|'+cur.chapterNo,rows=(index.groups&&index.groups[key])||[cur],targets={},seen={};
  rows.forEach(function(r){if(LANGS.indexOf(r.lang)<0||seen[r.lang])return;seen[r.lang]=1;targets[r.lang]=r.url});
  var languages=LANGS.filter(function(l){return !!seen[l]});
  return{current:cur,languages:languages,targets:targets,show:languages.length>1};
}
return{LANGS:LANGS,clean:clean,norm:norm,canon:canon,volNo:volNo,chNo:chNo,buildIndex:buildIndex,variantsForUrl:variantsForUrl};
});
