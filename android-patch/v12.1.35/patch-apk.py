...
NEW_VERSION='12.1.35'
NEW_CODE=51
OLD_PUSH='/* Nava v12.1.31 — Firestore-safe Android push token registration. */'
NEW_PUSH='/* Nava v12.1.35 — current Android push token registration. */'
...
def replace_push(js,push):
    ...
    if start<0: raise ValueError('old push block marker missing')
    end_marker='})(document,window);'
    end=js.find(end_marker,start)
    if end<0: raise ValueError('old push block end missing')
    end+=len(end_marker)
    out=js[:start]+push.rstrip()+js[end:]
    out=re.sub(r"appVersion:'\d+\.\d+\.\d+'", "appVersion:'12.1.35'", out)
    if NEW_PUSH not in out or "appVersion:'12.1.35'" not in out: raise ValueError('new push verify failed')
    if OLD_PUSH in out or '__navaAndroidPushV12131' in out: raise ValueError('old push block remains')
    return out
...
def main():
    if len(sys.argv)!=6:
        raise SystemExit('usage: patch-apk.py SRC PATCHED_DEX PUSH OUT EXPECTED_SHA')
    ...
    if b'NavaAndroidApp/12.1.35' not in dex: raise ValueError('current UA missing')
    if NEW_PUSH not in push or "appVersion:'12.1.35'" not in push: raise ValueError('push source invalid')
    ...
    with zipfile.ZipFile(srcp) as a, zipfile.ZipFile(out) as b:
        ...
        if not {'classes2.dex','assets/offline.html','assets/nava_app_v11.css'}.issubset(names): raise ValueError('preserved app entries missing')
        fj=b.read('assets/nava_app_v11.js').decode();fd=b.read('classes.dex')
        if NEW_PUSH not in fj or "appVersion:'12.1.35'" not in fj: raise ValueError('final push block missing')
        if '__navaAndroidPushV12131' in fj or "appVersion:'12.1.31'" in fj: raise ValueError('stale push registration remains')
        if b'NavaAndroidApp/12.1.35' not in fd: raise ValueError('final UA missing')
    ...
    print('PATCH_OK versionCode=51 versionName=12.1.35 push=12.1.35 startup-channel=native scoped=ok')
