...
if len(sys.argv) != 2:
    raise SystemExit('usage: patch-manifest77.py AndroidManifest.xml')
...
for value, start, length in strings:
    if value == '12.1.76':
        replacement = '12.1.77'.encode('utf-8' if utf8 else 'utf-16le')
        ...
        if size >= 20 and start + total * size <= position + chunk_size:
            for index in range(total):
                ...
                if name < count and strings[name][0] == 'versionCode':
                    if data[attribute + 15] != 0x10 or u32(attribute + 16) != 92:
                        raise ValueError('unexpected versionCode')
                    struct.pack_into('<I', data, attribute + 16, 93)
                    code_changes += 1
    position += chunk_size
if code_changes != 1:
...
print('MANIFEST_77_OK binary-axml versionName=12.1.77 versionCode=93')
