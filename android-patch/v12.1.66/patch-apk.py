...
OLD_VERSION='12.1.65';NEW_VERSION='12.1.66';NEW_CODE=82
SOURCE_SHA='b8e8e70995312ab952ff4d4db9436acb743266998a0ad835f7d3dbdc2e8fe8ae'
OLD_LIB='/* Nava Android 12.1.63 — relation-aware downloaded library: one Eser > Cilt > Bölüm tree. */'
NEW_LIB='/* Nava Android 12.1.66 — canonical relation-aware downloaded library: Eser > Cilt > Bölüm. */'
OLD_LANG='/* Nava Android 12.1.65 — restored per-series TR/EN/JP/KR/CN reader language engine. */'
NEW_LANG='/* Nava Android 12.1.66 — current-chapter-only reader language selector. */'
END='})(document,window);'
...
def main():
 if len(sys.argv)!=5:raise SystemExit('usage: patch-apk.py SRC LIB66 LANG66 OUT')
 src,libf,langf,out=sys.argv[1:];srcp=Path(src);got=hashlib.sha256(srcp.read_bytes()).hexdigest()
 if got.lower()!=SOURCE_SHA:raise ValueError('source must be signed Nava 12.1.65; sha256='+got)
 lib=Path(libf).read_text(encoding='utf-8');lang=Path(langf).read_text(encoding='utf-8')
 if NEW_LIB not in lib or NEW_LANG not in lang:raise ValueError('66 source marker missing')
 with zipfile.ZipFile(srcp) as zin:
  ...
  if js.count("seriesTitle:info.title+' Cilt '+v.volumeNo")<2:raise ValueError('legacy seriesTitle patch points missing')
  js=js.replace("seriesTitle:info.title+' Cilt '+v.volumeNo","seriesTitle:info.title",2)
  js=replace_iife(js,OLD_LIB,lib)
 ...
 with zipfile.ZipFile(out) as z:
  ...
  if "seriesTitle:info.title+' Cilt '+v.volumeNo" in fj:raise ValueError('legacy download metadata remains')
  for t in (NEW_LIB,'__navaDownloadedLibraryV12166','Promise.all([feed(\'Cilt\'),feed(\'Bölüm\')])',NEW_LANG,'currentAvailable','avail.length<=1','touch-action:pan-x','NavaSeriesLanguageV12149','nava-download-hub-v12149'):
   if t not in fj:raise ValueError('required token missing '+t)
 print('PATCH_OK versionName=12.1.66 versionCode=82 base=12.1.65 metadata=series-only library=feed-joined language=current-chapter native=byte-preserved')
