...
NEW_VERSION='12.1.40'
NEW_CODE=56
OLD_PUSH='/* Nava v12.1.39 — direct native FCM registration. */'
NEW_PUSH='/* Nava v12.1.40 — direct native FCM registration. */'
...
def main():
    if len(sys.argv)!=7: raise SystemExit('usage: patch-apk.py SRC DEX PUSH OFFLINE_HTML OUT EXPECTED_SHA')
    src,dexf,pushf,offlinef,out,expected=sys.argv[1:]
    ...
    dex=Path(dexf).read_bytes();push=Path(pushf).read_text();offline=Path(offlinef).read_bytes()
    if not dex.startswith(b'dex\n') or b'NavaAndroidApp/12.1.40' not in dex: raise ValueError('patched dex invalid')
    if NEW_PUSH not in push or "appVersion:'12.1.40'" not in push: raise ValueError('push source invalid')
    if b'<h1>\xc4\xb0ndirilenler</h1>' not in offline or b'Tekrar ba\xc4\x9flan' in offline or b'Yaln\xc4\xb1z Wi' in offline: raise ValueError('offline source invalid')
    with zipfile.ZipFile(srcp) as zin:
        names=set(zin.namelist())
        required={'AndroidManifest.xml','classes.dex','classes2.dex','classes3.dex','assets/nava_app_v11.js','assets/offline.html'}
        ...
        js=replace_block(zin.read('assets/nava_app_v11.js').decode('utf-8'),OLD_PUSH,push)
    ...
    allowed={'AndroidManifest.xml','classes.dex','assets/nava_app_v11.js','assets/offline.html'}
    with zipfile.ZipFile(srcp) as a,zipfile.ZipFile(out) as b:
        fj=b.read('assets/nava_app_v11.js').decode();fh=b.read('assets/offline.html').decode();fd=b.read('classes.dex')
        if NEW_PUSH not in fj or "appVersion:'12.1.40'" not in fj: raise ValueError('final push invalid')
        if 'İndirilenler' not in fh or 'Tekrar bağlan' in fh or 'Yalnız Wi' in fh or "seriesName(i)" not in fh: raise ValueError('offline redesign missing')
        for token in ('#6d28d9','#7c3aed','#8b5cf6','#5b20f3','#4c1d95','#c4b5fd','#ddd6fe','#ede9fe','#f5f3ff'):
            if token.lower() in fh.lower(): raise ValueError('purple token remains '+token)
        if b'NavaAndroidApp/12.1.40' not in fd: raise ValueError('UA missing')
    ...
    print('PATCH_OK versionCode=56 versionName=12.1.40 offline=grouped-library palette=blue-gray wifi_pref=download-center-only scoped=ok')
