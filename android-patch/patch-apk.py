...
def patch_manifest(raw, old_name='12.1.23', new_name='12.1.24', new_code=40):
...
def patch_js(original, replacement):
    start_marker = '/* Nava v12.1.22 — FREE push token bridge.'
    start = original.find(start_marker)
    if start < 0:
        raise ValueError('12.1.22 push bridge marker not found in APK asset')
    ...
    if "appVersion:'12.1.24'" not in patched:
        raise ValueError('12.1.24 token registration marker missing after patch')
    if "localStorage.getItem(key)==='1'" in patched:
        raise ValueError('stale localStorage registration gate still present')
    return patched
...
def main():
    ...
    with zipfile.ZipFile(target, 'r') as check:
        ...
        if "appVersion:'12.1.24'" not in js:
    ...
    print(f'PATCH_OK versionCode={new_code} js=12.1.24')
