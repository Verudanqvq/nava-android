...
OLD_VERSION='12.1.47';NEW_VERSION='12.1.52';NEW_CODE=68
...
FIX='/* Nava Android 12.1.52 — exact series-label volume/chapter discovery. */'
...
def main():
    ...
    with zipfile.ZipFile(srcp) as zin:
        ...
        if b'NavaAndroidApp/12.1.48' in dex or b'NavaAndroidApp/12.1.49' in dex or b'NavaAndroidApp/12.1.50' in dex or b'NavaAndroidApp/12.1.51' in dex or b'NavaAndroidApp/12.1.52' in dex: raise ValueError('unexpected newer UA marker')
    ...
    with zipfile.ZipFile(out) as z:
        ...
        if 'feedEntries(\'Cilt\'' in fj or 'feedEntries(\'Bölüm\'' in fj: raise ValueError('global feed discovery must not exist')
        if 'feedForLabel' not in fj or 'entryVolumeNo' not in fj: raise ValueError('exact series-label discovery missing')
        if b'NavaAndroidApp/12.1.47' not in fd or b'NavaAndroidApp/12.1.52' in fd: raise ValueError('native UA preservation failed')
        if CSS_MARK not in fc: raise ValueError('css patch missing')
    print('PATCH_OK versionName=12.1.52 versionCode=68 base=12.1.47 nativeUA=12.1.47 discovery=exact-series-label')
