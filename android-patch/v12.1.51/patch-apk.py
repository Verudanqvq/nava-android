...
OLD_VERSION='12.1.47';NEW_VERSION='12.1.51';NEW_CODE=67
...
FIX='/* Nava Android 12.1.51 — feed-backed series volume/chapter discovery hotfix. */'
...
def main():
    if len(sys.argv)!=9:
        raise SystemExit('usage: patch-apk.py SRC_12.1.47 MENU OFFLINE LANG NOTIFY CSS FEED_FIX OUT')
    src,menuf,offf,langf,notifyf,cssf,fixf,out=sys.argv[1:]
    ...
    fix=Path(fixf).read_text(encoding='utf-8')
    for text,mark in ((menu,NEW_MENU),(offline,NEW_OFFLINE),(lang,NEW_LANG),(notify,NOTIFY),(css,CSS_MARK),(fix,FIX)):
    ...
    with zipfile.ZipFile(srcp) as zin:
        ...
        if any(x in dex for x in (b'NavaAndroidApp/12.1.48',b'NavaAndroidApp/12.1.49',b'NavaAndroidApp/12.1.50',b'NavaAndroidApp/12.1.51')):
        ...
        if NOTIFY in js or FIX in js: raise ValueError('append patch already present')
        js=js.rstrip()+'\n\n'+notify.strip()+'\n\n'+fix.strip()+'\n'
    ...
    with zipfile.ZipFile(out) as z:
        ...
        for token in ('nava-series-download-inline-v12149','nava-download-hub-v12149','nava_download_queue_v12149','nava-series-language-v12149','nava-notification-clear-all-v12149','nava-offline-browser-v12149','navaVolumeFeedFixV12151'):
        ...
        if b'NavaAndroidApp/12.1.51' in fd: raise ValueError('12.1.51 UA must not be injected')
        if CSS_MARK not in fc: raise ValueError('css patch missing')
    print('PATCH_OK versionName=12.1.51 versionCode=67 base=12.1.47 nativeUA=12.1.47 download=feed-backed-volumes+chapters language=per-series notifications=delete offline=robust-delete')
