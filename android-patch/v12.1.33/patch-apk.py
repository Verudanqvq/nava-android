...
OLD_VERSION='12.1.32'; NEW_VERSION='12.1.33'; NEW_CODE=49
CSS_MARKER='/* Nava Android 12.1.33 — hard safe-area reset + compact shell. */'
...
def patch_manifest(raw):
 ...
 return bytes(d)
def patch_dex(raw):
 d=bytearray(raw);old=b'NavaAndroidApp/12.1.32';new=b'NavaAndroidApp/12.1.33';c=d.count(old)
 if c<1: raise ValueError('UA marker missing')
 d[:]=d.replace(old,new);d[12:32]=hashlib.sha1(d[32:]).digest();struct.pack_into('<I',d,8,zlib.adler32(d[12:])&0xffffffff);return bytes(d),c
def oldsig(name):
...
def main():
 ...
 src,cssf,out,expected=sys.argv[1:];srcp=Path(src);got=hashlib.sha256(srcp.read_bytes()).hexdigest()
 ...
 with zipfile.ZipFile(srcp) as zin:
  manifest=patch_manifest(zin.read('AndroidManifest.xml'));dex,ua_count=patch_dex(zin.read('classes.dex'));js=zin.read('assets/nava_app_v11.js').decode('utf-8')
  if "appVersion:'12.1.32'" not in js: raise ValueError('push appVersion marker missing')
  js=js.replace("appVersion:'12.1.32'","appVersion:'12.1.33'",1);oldcss=zin.read('assets/nava_app_v11.css').decode('utf-8')
 ...
 with zipfile.ZipFile(out) as z:
  ...
  assert b'NavaAndroidApp/12.1.33' in fd and fd[12:32]==hashlib.sha1(fd[32:]).digest();assert struct.unpack_from('<I',fd,8)[0]==(zlib.adler32(fd[12:])&0xffffffff);assert "appVersion:'12.1.33'" in fj and CSS_MARKER in fc
 print(f'PATCH_OK versionCode=49 versionName=12.1.33 ua_replacements={ua_count} css=safe-area-hard-reset scoped=ok')
