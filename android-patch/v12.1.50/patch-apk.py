...
OLD_VERSION='12.1.47';NEW_VERSION='12.1.50';NEW_CODE=66
...
def main():
    if len(sys.argv)!=8:
        raise SystemExit('usage: patch-apk.py SRC_12.1.47 MENU OFFLINE LANG NOTIFY CSS OUT')
    src,menuf,offf,langf,notifyf,cssf,out=sys.argv[1:]
    ...
    menu=Path(menuf).read_text(encoding='utf-8')
    offline=Path(offf).read_text(encoding='utf-8')
    lang=Path(langf).read_text(encoding='utf-8')
    notify=Path(notifyf).read_text(encoding='utf-8')
    css=Path(cssf).read_text(encoding='utf-8')
    ...
    with zipfile.ZipFile(srcp) as zin:
        ...
        # 12.1.48 and 12.1.49 both changed the native UA and both white-screened.
        # Keep the exact known-good 12.1.47 classes.dex/UA in 12.1.50.
        dex=zin.read('classes.dex')
        if b'NavaAndroidApp/12.1.47' not in dex: raise ValueError('known-good 12.1.47 UA missing')
        if b'NavaAndroidApp/12.1.48' in dex or b'NavaAndroidApp/12.1.49' in dex or b'NavaAndroidApp/12.1.50' in dex:
            raise ValueError('unexpected newer UA marker in 12.1.47 base')
        js=zin.read('assets/nava_app_v11.js').decode('utf-8')
        js=replace_iife(js,OLD_MENU,menu)
        js=replace_iife(js,OLD_OFFLINE,offline)
    ...
    with zipfile.ZipFile(out) as z:
        ...
        fc=z.read('assets/nava_app_v11.css').decode('utf-8')
        fd=z.read('classes.dex')
        ...
        if b'NavaAndroidApp/12.1.47' not in fd: raise ValueError('known-good UA not preserved')
        if b'NavaAndroidApp/12.1.50' in fd: raise ValueError('12.1.50 UA must not be injected')
        if CSS_MARK not in fc: raise ValueError('css patch missing')
    print('PATCH_OK versionName=12.1.50 versionCode=66 base=12.1.47 nativeUA=12.1.47 download=series-picker hub=queue+library language=per-series notifications=delete offline=robust-delete')
