#!/usr/bin/env bash
set -euo pipefail
STATUS=/tmp/status64.txt
: > "$STATUS"
printf 'status=running\nstarted=%s\n' "$(date -u +%FT%TZ)" >> "$STATUS"
trap 'rc=$?; if [ "$rc" -ne 0 ] && ! grep -q "^status=success$" "$STATUS"; then printf "status=failed\nfinished=%s\n" "$(date -u +%FT%TZ)" >> "$STATUS"; fi; exit "$rc"' EXIT
EXPECTED_SOURCE_SHA256=042029c2a68ac77169292d9ad20d7ad168e131367c90986505de6b894f9b23a4
EXPECTED_CERT_SHA256=acde7cf216852448a8a8277fe4bf11eac183394e6b34a862b124e693d51d09fe
SOURCE_TAG=v12.1.63
TARGET_TAG=v12.1.64
python -m py_compile android-patch/v12.1.64/patch-apk.py
node --check android-patch/v12.1.64/library-overlay-fix.js
grep -q '#nava-offline-browser-v12163' android-patch/v12.1.64/ui-v12164.css
printf 'source_validation=ok\nbase=12.1.63\nfix=library-overlay-root\nscroll_fail_safe=ok\nnative=byte-preserved\n' >> "$STATUS"
set +x
...
python android-patch/v12.1.64/patch-apk.py /tmp/current/Nava.apk android-patch/v12.1.64/library-overlay-fix.js android-patch/v12.1.64/ui-v12164.css /tmp/Nava-unsigned.apk | tee /tmp/patch64.txt
grep -q 'PATCH_OK versionName=12.1.64 versionCode=80' /tmp/patch64.txt
...
unzip -p /tmp/Nava-unsigned.apk assets/nava_app_v11.js > /tmp/app64.js
unzip -p /tmp/Nava-unsigned.apk assets/nava_app_v11.css > /tmp/app64.css
node --check /tmp/app64.js
grep -q '__navaLibraryOverlayFixV12164' /tmp/app64.js
grep -q '#nava-offline-browser-v12163\[hidden\]' /tmp/app64.css
BUILD_TOOLS="$(find "$ANDROID_HOME/build-tools" -mindepth 1 -maxdepth 1 -type d | sort -V | tail -1)"
"$BUILD_TOOLS/aapt" dump badging /tmp/Nava-unsigned.apk | grep -q "versionCode='80'.*versionName='12.1.64'"
...
"$BUILD_TOOLS/apksigner" verify --verbose --print-certs /tmp/Nava.apk > /tmp/verify64.txt
CERT="$(grep -i -m1 'certificate SHA-256 digest:' /tmp/verify64.txt | sed 's/.*digest:[[:space:]]*//' | tr -d ':[:space:]' | tr '[:upper:]' '[:lower:]')"
...
if gh release view "$TARGET_TAG" --repo "$GITHUB_REPOSITORY" >/dev/null 2>&1; then
 ...
 gh release edit "$TARGET_TAG" --repo "$GITHUB_REPOSITORY" --title 'Nava 12.1.64' --latest
 printf 'release=v12.1.64\nasset=Nava.apk\nrelease_existing=yes\n' >> "$STATUS"
else
 NOTES='12.1.64 fixes the downloaded-library overlay regression in 12.1.63. The v12163 library root now has its own fullscreen fixed CSS instead of inheriting an ID-specific v12162 rule. A click fail-safe opens the active library renderer and removes the page scroll lock if the overlay does not become visible. 12.1.63 hierarchy, relation cache, speed improvements and native wake-lock background runtime are preserved byte-for-byte.'
 gh release create "$TARGET_TAG" /tmp/Nava.apk#Nava.apk --repo "$GITHUB_REPOSITORY" --title 'Nava 12.1.64' --notes "$NOTES" --latest
 printf 'release=v12.1.64\nasset=Nava.apk\nrelease_existing=no\n' >> "$STATUS"
fi
