#!/usr/bin/env bash
set -euo pipefail
STATUS=/tmp/status66.txt
: > "$STATUS"
printf 'status=running\nstarted=%s\n' "$(date -u +%FT%TZ)" >> "$STATUS"
trap 'rc=$?; if [ "$rc" -ne 0 ] && ! grep -q "^status=success$" "$STATUS"; then printf "status=failed\nfinished=%s\n" "$(date -u +%FT%TZ)" >> "$STATUS"; fi; exit "$rc"' EXIT
EXPECTED_SOURCE_SHA256=b8e8e70995312ab952ff4d4db9436acb743266998a0ad835f7d3dbdc2e8fe8ae
EXPECTED_CERT_SHA256=acde7cf216852448a8a8277fe4bf11eac183394e6b34a862b124e693d51d09fe
SOURCE_TAG=v12.1.65
TARGET_TAG=v12.1.66
python -m py_compile android-patch/v12.1.66/patch-apk.py
node --check android-patch/v12.1.66/downloaded-library-v12166.js
node --check android-patch/v12.1.66/language-v12166.js
printf 'source_validation=ok\nbase=12.1.65\nfix=canonical-series-metadata-plus-feed-join\nlanguage=current-chapter-only\nnative=byte-preserved\n' >> "$STATUS"
set +x
...
python android-patch/v12.1.66/patch-apk.py /tmp/current/Nava.apk android-patch/v12.1.66/downloaded-library-v12166.js android-patch/v12.1.66/language-v12166.js /tmp/Nava-unsigned.apk | tee /tmp/patch66.txt
grep -q 'PATCH_OK versionName=12.1.66 versionCode=82' /tmp/patch66.txt
...
unzip -p /tmp/Nava-unsigned.apk assets/nava_app_v11.js > /tmp/app66.js
node --check /tmp/app66.js
! grep -Fq "seriesTitle:info.title+' Cilt '+v.volumeNo" /tmp/app66.js
grep -Fq '__navaDownloadedLibraryV12166' /tmp/app66.js
grep -Fq "Promise.all([feed('Cilt'),feed('Bölüm')])" /tmp/app66.js
grep -Fq 'nava-reader-language-v12166' /tmp/app66.js
grep -Fq 'avail.length<=1' /tmp/app66.js
grep -Fq 'touch-action:pan-x' /tmp/app66.js
BUILD_TOOLS="$(find "$ANDROID_HOME/build-tools" -mindepth 1 -maxdepth 1 -type d | sort -V | tail -1)"
"$BUILD_TOOLS/aapt" dump badging /tmp/Nava-unsigned.apk | grep -q "versionCode='82'.*versionName='12.1.66'"
printf 'patch=ok\nversionName=12.1.66\nversionCode=82\nmetadata_series_only=ok\nlegacy_relation_join=ok\nlanguage_current_chapter=ok\nclasses1_preserved=ok\nclasses2_preserved=ok\nclasses3_preserved=ok\nresources_preserved=ok\ncss_preserved=ok\noffline_preserved=ok\n' >> "$STATUS"
...
"$BUILD_TOOLS/apksigner" verify --verbose --print-certs /tmp/Nava.apk > /tmp/verify66.txt
CERT="$(grep -i -m1 'certificate SHA-256 digest:' /tmp/verify66.txt | sed 's/.*digest:[[:space:]]*//' | tr -d ':[:space:]' | tr '[:upper:]' '[:lower:]')"
...
if gh release view "$TARGET_TAG" --repo "$GITHUB_REPOSITORY" >/dev/null 2>&1; then
 ...
 gh release edit "$TARGET_TAG" --repo "$GITHUB_REPOSITORY" --title 'Nava 12.1.66' --latest
 printf 'release=v12.1.66\nasset=Nava.apk\nrelease_existing=yes\n' >> "$STATUS"
else
 NOTES='12.1.66 fixes downloaded hierarchy metadata and reader language visibility. New volume/chapter downloads store the real work title instead of “Work Cilt N”. Existing legacy downloads are remapped by joining live Cilt and Bölüm feeds by URL/volume relation so volumes stay under their work and the tree remains Eser → Cilt → Bölüm. Reader language controls now show only languages available for the current chapter, disappear when only one language exists, and support horizontal touch scrolling. 12.1.63 wake-lock/speed runtime, 12.1.61 startup and 12.1.60 updater are preserved.'
 gh release create "$TARGET_TAG" /tmp/Nava.apk#Nava.apk --repo "$GITHUB_REPOSITORY" --title 'Nava 12.1.66' --notes "$NOTES" --latest
 printf 'release=v12.1.66\nasset=Nava.apk\nrelease_existing=no\n' >> "$STATUS"
fi
